...
class comandoDocumentoSesion
{
    ...
    /**
     * @nombre :lista
     * @descripcion:consulta en bd acuerdos/compromisos.
     */
    function TablaAcuerdosCompromisos($p)
    {

        try {
            $Conexion = Conexion::getInstance()->obtenerConexion();
            $this->SQL = "SELECT         acuerdo_compromiso.titulo, 
                            acuerdo_compromiso.cve_tipo, 
                            tipo.nombre, 
                            acuerdo_compromiso.descripcion, 
                            acuerdo_compromiso.cve_estatus_compromiso,
                            acuerdo_compromiso.cve_acuerdo_compromiso,
                            acuerdo_compromiso.fecha_cumplimiento
                            FROM            acuerdo_compromiso 
                            INNER JOIN tipo ON acuerdo_compromiso.cve_tipo = tipo.cve_tipo
                          WHERE        (acuerdo_compromiso.cve_sesion = :cve_sesion) AND (tipo.activo = 1) ";
